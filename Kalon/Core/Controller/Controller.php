...
class Controller extends Object
{
  ...
  public $actionName = '';
	...
	public function loadModel($modelClass){
		if(file_exists(APP.'Models/'.$modelClass.'.php')){
			require APP.'Models/'.$modelClass.'.php';
	...
	public function loadModels()
	{
		if(is_bool($this->uses) || (is_array($this->uses) && sizeof($this->uses)>0)){
			...
			require_once(KALON . 'Core/Model/Model.php');
			if(file_exists(APP . 'AppModel.php')){
				require_once(APP . 'AppModel.php');
			}
			else {
				require_once(KALON . 'Core/Model/AppModel.php');
			}
			if($this->uses === false){
