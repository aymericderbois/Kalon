...
class View {
	...
	public function render($filename) {
		...
		$viewFile = APP . 'Views' . DS . $this->controller->controllerName . DS . ucfirst($this->controller->actionName) . '.ctp';
		if (file_exists($viewFile)) {
			...
			if ($this->controller->layout == null) {
				...
				$layoutFile = APP . 'Views' . DS . 'Layouts' . DS . $this->controller->layout . '.ctp';
	...
	public function loadHelpers() {
		...
		foreach ($this->controller->helpers as $helper) {
			if (file_exists(APP . 'Views' . DS . 'Helpers' . DS . $helper . 'Helper.php')) {
				require(APP . 'Views' . DS . 'Helpers' . DS . $helper . 'Helper.php');
				$helperClassName = $helper . 'Helper';
				$this->{$helper} = new $helperClassName();
			} else if (file_exists(KALON . 'Core' . DS . 'View' . DS . 'Helpers' . DS . $helper . 'Helper.php')) {
				require(KALON . 'Core' . DS . 'View' . DS . 'Helpers' . DS . $helper . 'Helper.php');
				$helperClassName = $helper . 'Helper';
				$this->{$helper} = new $helperClassName();
			} else {
	...
	public function element($name, $vars = array()) {
		$filename = $this->controller->app . 'Elements' . DS . $name . '.php';
